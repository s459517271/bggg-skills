...
> **随 Sif 关键词 Skills 分发包打包的第三方组件**  
> 上游项目：[github.com/eze-is/web-access](https://github.com/eze-is/web-access) · 作者：[一泽 Eze](https://github.com/eze-is) · 协议：MIT · [官网](https://web-access.eze.is)  
> 本目录内容为上游 Skill 的副本，用于 Sif 方式 B（浏览器导出）；更新请从上游仓库获取。完整说明见下文。
...
**v2.5.3 更新：**
- **URL 通过 POST body 传递** — `/new` 与 `/navigate` 不再把目标 URL 放入 query，避免目标 URL 自带 query 时丢失 token 或被错误切分。
